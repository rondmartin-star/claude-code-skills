...
```
### Windows Command Execution from Bash/Claude (CRITICAL)

When orchestrating Windows tools (WiX, MSI, candle.exe, light.exe) from bash or Claude:

**❌ WRONG: Try to run Windows commands directly from bash**
```bash
# Fails: heat.exe not in PATH
heat.exe dir "C:\App" -out files.wxs

# Creates literal "nul" file instead of redirecting
del /Q *.obj >nul 2>&1

# PowerShell $PSScriptRoot is empty when called from bash
powershell -File build-msi.ps1

# Path quoting issues
cd "C:\Program Files\App"  # Works in cmd but may fail in bash
```

**✅ RIGHT: Use native Windows batch/PowerShell scripts**
```batch
REM create-msi.bat - All Windows tooling orchestrated in batch
@echo off
setlocal EnableDelayedExpansion
cd /d "%~dp0"

set "WIX_BIN=C:\Program Files (x86)\WiX Toolset v3.14\bin"
"%WIX_BIN%\heat.exe" dir "%~dp0.." -out files.wxs
"%WIX_BIN%\candle.exe" -arch x64 files.wxs
"%WIX_BIN%\light.exe" -out app.msi files.wixobj

REM Cleanup (use 2>NUL not 2>&1)
del /Q *.wixobj 2>NUL
```
...
1. **Redirect Syntax**
   - Bash: Use `2>/dev/null` for error suppression
   - Batch: Use `2>NUL` (not `>nul 2>&1` which creates a "nul" file when run from bash)
   - PowerShell: Use `-ErrorAction SilentlyContinue`

2. **PowerShell from Bash**
   ```bash
   # WRONG - $PSScriptRoot is empty
   powershell -File script.ps1

   # RIGHT - use explicit paths or cmd wrapper
   powershell -Command "& { Set-Location 'C:\App'; .\script.ps1 -Path 'C:\App' }"

   # BEST - use batch wrapper
   ./run-build.bat  # batch script calls PowerShell internally
   ```

3. **Path Quoting**
   ```bash
   # Bash on Windows - use forward slashes or escape spaces
   cd /c/Program\ Files/App     # Works
   cd "C:\Program Files\App"    # May fail in bash

   # Batch - use quotes for spaces
   cd /d "C:\Program Files\App"  # Always works
   ```

4. **Tool Not Found**
   ```bash
   # WRONG - assumes tool in PATH
   candle.exe file.wxs

   # RIGHT - use full path or batch script
   "C:\Program Files (x86)\WiX Toolset v3.14\bin\candle.exe" file.wxs

   # BEST - batch script with proper path detection
   ```

5. **Build Orchestration Pattern**
   ```batch
   REM build-msi.bat - Handles all steps, callable from anywhere
   @echo off
   setlocal EnableDelayedExpansion
   cd /d "%~dp0"
   ...
   REM Step 2: Harvest
   "%WIX_BIN%\heat.exe" dir "%SOURCE%" -out ApplicationFiles.wxs

   REM Step 3: Fix references (if Python script exists)
   if exist fix-refs.py python fix-refs.py

   REM Step 4: Compile
   "%WIX_BIN%\candle.exe" -arch x64 App.wxs ApplicationFiles.wxs

   REM Step 5: Link
   "%WIX_BIN%\light.exe" -out App.msi *.wixobj

   echo Build complete: App.msi
   ```

**Rule:** For Windows-specific tooling (WiX, MSI, Windows services, etc.), create comprehensive batch/PowerShell scripts instead of trying to orchestrate individual commands through bash. Let native Windows scripts handle path quirks, redirects, and tool locations.

**When Claude is helping:**
- Create a batch script with all build steps
- Claude runs: `./build-script.bat`
- Script handles all Windows-specific complexity
- Avoids bash/Windows impedance mismatch
